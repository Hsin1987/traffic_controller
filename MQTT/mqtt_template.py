...
class MQTT_OBJ():
    def __init__(self, client_id="service_provider", broker_ip="iot.eclipse.org", port=1883, keepalive=10,
                 clean_session=False, logger=None):
        # -----   Member variable --------#
        self.logger = logger
        # NO logger specified
    ...
    def publish(self, topic, payload, qos, retain):
        ...
        if self.available == "offline" and (not IS_PUB_WITHOUT_CONNECT):
            ...
            if IS_PUB_LOG:
                self.logger.info("[MQTT] publish '" + str(payload) + "' to '" + topic + "' (Q" + str(qos) + ", R" + str(
                    int(retain)) + ", Mid: " + str(pub_rc[1]) + ")")
            if pub_rc[0] != mqtt.MQTT_ERR_SUCCESS:  # Something wrong
                self.logger.error("[MQTT] publish " + mqtt.error_string(pub_rc[0]) + " (Mid: " + str(
    ...
    def publish_blocking(self, topic, payload, qos, retain, timeout=10):
        ...
        if self.available == "offline" and (not IS_PUB_WITHOUT_CONNECT):
            ...
            while not pub_rc.is_published():
                ...
                time.sleep(0.01)
            if IS_PUB_LOG:
                self.logger.info("[MQTT] publish completed in " + str(
                    round((time.time() - t_start) * 1000, 3)) + " ms" + ", (Mid: " + str(pub_rc[1]) + ")")
            return (1, pub_rc)
    ...
    def on_message(self, client, userdata, message):
        ...
        self.logger.info("[MQTT] on_message :  " + str(message.payload) + "(Q" + str(message.qos) + ", R" + str(
            message.retain) + ")")
